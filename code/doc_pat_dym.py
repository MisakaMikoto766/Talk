...
class BBN:
    def __init__(self,
        ...
        self.init_prompt()
        with open("spikes_strategies.json", "r", encoding="utf-8") as f:
            self.spikes_map = json.load(f)
        self.save_file["SPIKES"] = {}
        self.creat_agents()
        self.init_agents()
    ...
    def set_single_turn_context(self, player: BBNPlayer, meta_prompt: str, event_prompt: str):

        player.memory_lst = []  
        player.set_meta_prompt(meta_prompt) 
        player.add_event(event_prompt)  
    ...
    def run(self):
        ...
        for round in range(self.max_round - 1):
            if self.mod_ans["Summary of Informing Situation"] != '':
                ...
                self.planner.add_event(self.save_file['planner_system_prompt'].replace('##dialog_history##', self.save_file['dialog_history']))
                self.pla_ans = self.planner.ask()
                self.planner.add_memory(self.pla_ans)

                self.pla_ans = self.pla_ans.replace('```json', '').replace('```', '').strip()
                self.pla_ans = json.loads(self.pla_ans)
                strategy_key = self.pla_ans["selected_strategy"]
                strategy_text = self.spikes_map.get(strategy_key, "")



                print(f"===== Conversation Round-{round+2} =====\n")
                doctor_prompt = self.save_file['doctor_prompt'] \
                    .replace('##spikes##', strategy_text) \
                ...
                self.save_file["SPIKES"][f"round {round+2}"] = strategy_key
...
if __name__ == "__main__":
    ...
    config = json.load(open(f"{MAD_path}/code/utils/config4pd_name.json", "r"))
    ...
    for id, data in enumerate(tqdm(inputs)):
        ...
        try:
            bbn = BBN(save_file_dir=save_file_dir, num_players=4, model_name=model_name,
